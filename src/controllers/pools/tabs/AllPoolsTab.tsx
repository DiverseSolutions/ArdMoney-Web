...
export default function AllPoolsTab({ search }: { search: string }) {
  const { t } = useTranslation("pools");
  const { data: pools, isSuccess, isLoading } = useAllPoolsQuery();
          ...
          <PoolTable>
            {filteredPools.map((pool: any, index: number) => {
              const baseToken = dexList.find(
                (token) => token.symbol === pool.token0.symbol
              );
              const quoteToken = dexList.find(
                (token) => token.symbol === pool.token1.symbol
              );
