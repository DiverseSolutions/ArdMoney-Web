		...
		<div className="flex justify-center min-h-screen w-full ">
			<div className='flex flex-col w-full max-w-6xl mb-xl z-10 p-6 lg:p-0'>
				...
				<div className='flex gap-sm text-white w-full lg:w-1/3'>
					{tabs.map((item, index) => (
						<button onClick={() => clickTab(index)} key={index} className={`${index === selectedIndex ? "bg-primary" : "bg-primary/10"}  py-sm w-1/2 text-center rounded-lg`}>
							<span>{item.text}</span>
						</button>
					))}
				</div>
					...
					<div className='border border-primary/30 rounded-lg p-3 justify-start block w-full overflow-auto text-left box-border mt-lg'>
						<table className='w-full'>
							...
							<tbody >
								<>
									{body.map((item, index) => (
										<tr key={index} className='text-white border-t border-primary/30 '>
											...
											<td className='px-6'>{item.merged_transaction} transfer</td>
											<td className='px-6'>{item.burned_transaction} transfer</td>
										</tr>
									))}
								</>
							</tbody>
